...
function Reset() {
  //función que inicializa las variables para los puntos y adivinanzas disponibles
...
function Init() {
  //escoge una adivinanza al azar para iniciar el juego
  ...
  methods: {
    Save_User() {
      ...
      if (this.year < 2024 && this.year > 1) {
        ...
        swal("", "Registra un año válido", "error");
      }
    },
    Save_Player() {
    ...
    Play() {
      //esta función se usa para comprobar si la persona adivinó
      let user = JSON.parse(localStorage.getItem("user"));
      user.Year == this.year ? this.Win() : this.Calc(user);
    },
    Win() {
      //en caso de adivinar se resetea las vidas y el score y se aumenta la puntucación
      this.lives = 7;
      this.score += 10;
      this.percent = 0;
